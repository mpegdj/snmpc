...
## Discovery에서 MVE5000/MVD5000 Trap 설정 기능 추가 계획 (2025-12-26)

### 목적

Discovery 완료 후 MVE5000/MVD5000 기기에 자동으로 Trap Destination을 설정할 수 있도록 기능 추가.

### 현재 문제점

1. **표준 OID만 지원**: 현재는 표준 OID (`1.3.6.1.6.3.18.1.3.0`)만 시도하지만 MVE5000/MVD5000은 지원하지 않음
2. **MVE5000/MVD5000 전용 설정 없음**: Trap Destination Table을 사용하는 기기에 대한 설정 기능 없음
3. **SNMP 지원 기기 구분 없음**: Discovery에서 SNMP 지원 여부를 명확히 표시하지 않음

### 구현 계획

#### Phase 1: Discovery UI 개선 ✅

**1. DataGrid에 "SNMP" 체크박스 컬럼 추가**
- 파일: `SnmpNms.UI/Views/Dialogs/DiscoveryProgressDialog.xaml`
- "Add" 컬럼 다음에 "SNMP" 체크박스 컬럼 추가
- SNMP 지원 기기만 자동 체크 (기본값)
- 사용자가 수동으로 언체크 가능

**2. DiscoveredDevice 모델 확장**
- 파일: `SnmpNms.UI/Views/Dialogs/DiscoveryProgressDialog.xaml.cs`
- `IsSnmpSupported` 속성 추가
- Discovery 완료 후 `Status == "SNMP OK"`인 기기는 `IsSnmpSupported = true`

**3. "Standard NTT" 체크박스 추가**
- 파일: `SnmpNms.UI/Views/Dialogs/DiscoveryProgressDialog.xaml`
- "Configure Trap Destination" 체크박스 아래에 추가
- 디폴트: Unchecked
- MVE5000/MVD5000 전용 Trap 설정 옵션

#### Phase 2: MVE5000/MVD5000 Trap 설정 로직 구현 ✅

**4. ConfigureStandardNttTrapAsync 메서드 추가**
- 파일: `SnmpNms.UI/Views/Dialogs/DiscoveryPollingAgentsDialog.xaml.cs`
- MVE5000/MVD5000 Trap Destination Table 설정
- GET으로 1번 Entry 확인 후 중복 스킵
- SET으로 1번 Entry 설정 (Community: "public", IP: NMS 서버 IP)

**5. OK 버튼 클릭 시 Trap 설정 실행**
- 파일: `SnmpNms.UI/Views/Dialogs/DiscoveryPollingAgentsDialog.xaml.cs`
- "Standard NTT" 체크되어 있으면 선택된 SNMP 지원 기기에 Trap 설정
- 실행 시점: OK 버튼 클릭 시 (마지막)

### 구현 세부사항

#### MVE5000/MVD5000 Trap Destination Table 구조

**OID 구조**:
- MVE5000: `1.3.6.1.4.1.3930.36.5.2.11` (mve5000SnmpTrapDstTable)
- MVD5000: `1.3.6.1.4.1.3930.35.5.2.11` (mvd5000SnmpTrapDstTable)

**Entry 구조 (Index 1-8)**:
- Index: `{base}.1.1.{index}` (read-only)
- ServiceEnable: `{base}.1.2.{index}` (0=enabled, 1=disabled)
- CommunityName: `{base}.1.3.{index}` (DisplayString)
- Protocol: `{base}.1.4.{index}` (0=IPv4, 1=IPv6)
- IPv4Address: `{base}.1.5.{index}` (IpAddress)
- IPv6Address: `{base}.1.6.{index}` (Ipv6Address)

#### Trap 설정 로직

**1. 기기 확인**:
- MVE5000/MVD5000 Table Walk로 기기 타입 확인
- Table이 없으면 스킵 (해당 기기가 아님)

**2. 중복 체크**:
- 1번 Entry GET으로 확인
- ServiceEnable이 enabled(0)이고 IPv4Address가 설정되어 있으면 스킵

**3. Trap 설정**:
- ServiceEnable = 0 (enabled)
- CommunityName = "public"
- Protocol = 0 (IPv4)
- IPv4Address = NMS 서버 IP

#### 실행 시점

**OK 버튼 클릭 시 (마지막)**:
- 장점: GET 없이 바로 SET 가능 (빠름)
- 중복 체크: GET으로 확인 후 스킵 (불필요한 SET 방지)
- 사용자가 선택한 기기에만 적용

### 구현 파일

1. ✅ `SnmpNms.UI/Views/Dialogs/DiscoveryProgressDialog.xaml` (수정)
2. ✅ `SnmpNms.UI/Views/Dialogs/DiscoveryProgressDialog.xaml.cs` (수정)
3. ✅ `SnmpNms.UI/Views/Dialogs/DiscoveryPollingAgentsDialog.xaml.cs` (수정)
...
1. Discovery 실행
   ↓
2. 기기 발견 (SNMP OK인 기기는 IsSnmpSupported = true, IsSelected = true)
   ↓
3. "Standard NTT" 체크박스 선택 (옵션)
   ↓
4. OK 버튼 클릭
   ↓
5. Map에 기기 등록
   ↓
6. "Standard NTT" 체크되어 있으면:
   - 선택된 SNMP 지원 기기에 대해
   - GET으로 1번 Entry 확인
   - 이미 설정되어 있으면 스킵
   - 설정되어 있지 않으면 SET으로 설정
```

### 주의사항

1. **Community**: Trap Community는 "public" 사용 (Read Community와 동일)
2. **Port**: MVE5000/MVD5000 Table에는 Port가 없으므로 일반 Trap Port(162) 사용
3. **중복 체크**: GET으로 확인하여 불필요한 SET 방지
4. **개별 편집**: Device Property Dialog의 Trap 탭에서 개별 편집 가능 (향후 구현)

### 향후 확장

1. **개별 편집**: MapObjectPropertiesDialog의 Trap 탭에서 Trap Destination List 편집
2. **다중 Entry**: 여러 Trap Destination 설정 지원
3. **Community 선택**: Trap Community를 사용자가 선택할 수 있도록

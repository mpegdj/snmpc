...
**2. Producer-Consumer 기반 트랩 엔진 (진행 중)**
*   **목표**: UDP 리스너 스레드는 '수신'만 담당하여 큐에 쌓고, 별도의 '워커 스레드'들이 큐에서 데이터를 꺼내 파싱 및 상태 업데이트를 수행하는 구조.
*   **예상 효과**: 트랩 폭주(Trap Storm) 발생 시에도 리스너가 블로킹되지 않아 패킷 유실을 원천 차단.

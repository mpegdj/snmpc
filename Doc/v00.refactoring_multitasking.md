...
### Phase 3: UI 및 리소스 최적화 (진행 중)

**1. UI Throttling (Batch 처리) - 진행 중**
*   **구현 내용**: 초당 수백 개의 트랩이 발생할 때마다 UI를 직접 갱신하지 않고, **0.1~0.2초 주기(Flush Interval)**로 누적된 변경 사항을 한 번에 반영함. 
*   **기술적 이점**: 
    *   **UI 스레드 부하 감소**: Dispatcher를 호출하는 빈도를 1/N 수준으로 줄여 UI 프리징을 원천 차단.
    *   **렌더링 최적화**: 잦은 레이아웃 재계산을 방지하고 모아서 한 번에 그림.
*   **주요 코드**:
    ```csharp
    // EventHandlingService.cs
    private void StartUiThrottlingTimer() {
        _timer = new DispatcherTimer { Interval = TimeSpan.FromMilliseconds(150) };
        _timer.Tick += (s, e) => FlushUpdatesToUi();
        _timer.Start();
    }
    ```

**2. 상태 전파(Status Propagation) 최적화 - 진행 중**
*   **구현 내용**: 장비 상태가 변할 때마다 트리 전체를 재계산(Recursive Recompute)하던 방식을 개선. 상태 변경이 발생한 노드에 'Dirty' 마크를 남기고 렌더링 주기(Throttling 루프)에서 한 번만 전체 트리를 갱신함.
*   **예상 효과**: 트리의 깊이(Depth)와 관계없이 고정된 부하만 발생.

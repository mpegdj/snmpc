...
### 2025-12-28 (restore 실행)

- `recover/discovery-gui-gonextwalk` 브랜치에서 아래 4개 파일만 `rescue/backup-20251228` 기준으로 가져옴(`git restore --source ... --staged`)
- staged 변경 목록 확인 결과(금지 파일 0개):
  - `SnmpNms.Infrastructure/SnmpClient.cs` (M)
  - `SnmpNms.UI/Views/Dialogs/DiscoveryPollingAgentsDialog.xaml.cs` (M)
  - `SnmpNms.UI/Views/Dialogs/DiscoveryProgressDialog.xaml` (M)
  - `SnmpNms.UI/Views/Dialogs/DiscoveryProgressDialog.xaml.cs` (M)

- 빌드 에러 대응(Discovery → UiSnmpTarget 의존성)
  - 증상: `DiscoveryPollingAgentsDialog.xaml.cs`에서 `UiSnmpTarget.SysObjectId` 사용으로 컴파일 오류 발생(CS0117)
  - 조치: `SnmpNms.UI/Models/UiSnmpTarget.cs`에 `SysObjectId` 문자열 속성 1개 추가(데이터 보관용)
  - 주의: MIB 파싱/필터링(`MibService.cs`)과 무관한 변경
